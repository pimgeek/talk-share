require 'mina/bundler'
require 'mina/rails'
require 'mina/git'

set :domain, '106.186.119.248'
set :deploy_to, '/web/talk-share'
set :current_path, 'current'
set :repository, 'git://github.com/arlyxiao/talk-share.git'
set :branch, 'master'
set :user, 'root'

set :shared_paths, [
  'config/share_tags.yaml',
  'config/secrets.yml',
  'tmp',
  'log',
  'db'
]

task :environment do
end

task :setup => :environment do
  queue! %[mkdir -p "#{deploy_to}/shared/tmp/sockets"]
  queue! %[chmod g+rx,u+rwx "#{deploy_to}/shared/tmp/sockets"]

  queue! %[mkdir -p "#{deploy_to}/shared/tmp/pids"]
  queue! %[chmod g+rx,u+rwx "#{deploy_to}/shared/tmp/pids"]

  queue! %[mkdir -p "#{deploy_to}/shared/tmp/logs"]
  queue! %[chmod g+rx,u+rwx "#{deploy_to}/shared/tmp/logs"]

  queue! %[mkdir -p "#{deploy_to}/shared/config"]
  queue! %[chmod g+rx,u+rwx "#{deploy_to}/shared/config"]
  queue! %[touch "#{deploy_to}/shared/config/secrets.yml"]
  queue! %[touch "#{deploy_to}/shared/config/share_tags.yaml"]

  queue! %[mkdir -p "#{deploy_to}/shared/db"]
  queue! %[chmod g+rx,u+rwx "#{deploy_to}/shared/db"]
  

  queue! %[mkdir -p "#{deploy_to}/shared/log"]
  queue! %[chmod g+rx,u+rwx "#{deploy_to}/shared/log"]

  queue  %[echo "-----> Be sure to edit 'shared/config/share_tags.yaml'."]
  queue  %[echo "-----> Be sure to edit 'shared/config/secrets.yml'."]
end

desc "Deploys the current version to the server."
task :deploy => :environment do
  deploy do
    invoke :'git:clone'
    invoke :'deploy:link_shared_paths'
    queue! "bundle"
    queue! "RAILS_ENV=production rake db:migrate"
    invoke :'rails:assets_precompile'

    to :launch do
      queue %[
        source /etc/profile
        ./deploy/sh/unicorn.sh stop
        ./deploy/sh/unicorn.sh start
      ]
    end
  end
end

desc "update code only"
task :update_code => :environment do
  deploy do
    invoke :'git:clone'
    invoke :'deploy:link_shared_paths'
    queue! "bundle"
    queue! "RAILS_ENV=production rake db:migrate"
    invoke :'rails:assets_precompile'
  end
end

desc "restart server"
task :restart => :environment do
  queue %[
    source /etc/profile
    cd #{deploy_to}/#{current_path}
    ./deploy/sh/unicorn.sh stop
    ./deploy/sh/unicorn.sh start
  ]
end

